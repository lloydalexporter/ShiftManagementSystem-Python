...
class DatabaseManagement:
	...
	def saveData(self, dataArray, CURRENT_YEAR, CURRENT_MONTH):
		...
		if self.ifMain: print("month =", month)
		
		if CURRENT_MONTH == "12": # If the actual month is December ...
			if month == "01": # ... and the month we're adding is January ...
				CURRENT_YEAR = str(int(CURRENT_YEAR) + 1) # ... then add one to the year.
		
		tableName = CURRENT_YEAR + "_" + month # Format the table name from the year and month.
		if self.ifMain: print("tableName =", tableName)
